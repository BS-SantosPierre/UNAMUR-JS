...
function refreshHistory() {
	...
	for (const { icon, city, temp } of weatherHistory) {
		...
		wrapper.className = 'weather-display'
		// Img
		...
		wrapperContent.className = 'content';
		const spanCity = document.createElement('span');
		spanCity.className = 'name';
		spanCity.innerHTML = city;
		const spanTemp = document.createElement('span');
		spanTemp.className = 'temp';
		spanTemp.innerHTML = `${temp}°C`;
		wrapperContent.appendChild(spanCity);
		wrapperContent.appendChild(spanTemp);

		wrapper.appendChild(wrapperImg);
		wrapper.appendChild(wrapperContent);

		historyElement.appendChild(wrapper);
	}
